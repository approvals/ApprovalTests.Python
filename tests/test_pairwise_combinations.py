...
def add_pair(in1, in2, all_pairs_with_index):
    key = get_key(in1, in2)
...
def assert_all_pairs_present_between_lists(input1:Sequence[Any], input2:Sequence[Any], all_pairs_with_index: Dict[str, int]):
    for i1 in input1:
        for i2 in input2:
           key = get_key(i1, i2)
...
def assert_all_pairs_present(all_pairs_with_index: Dict[str, int], inputs: Sequence[Sequence[Any]]):
    for i1 in range(len(inputs) - 1):
        for i2 in range(i1 + 1, len(inputs)):
            assert_all_pairs_present_between_lists(inputs[i1], inputs[i2], all_pairs_with_index)


def assert_pairwise_combinations(inputs: Sequence[Sequence[Any]]):
